...
model = joblib.load('C:/Users/kiran/Downloads/transfer.pcloud.yCpr95sG/CODE/CODE/Deploy/latest/new/RFC.pkl')
...
def predict_view(request):
    if request.method == 'POST':
        print('IF')
        fieldss = ['TENURE_IN_MONTHS', 'OFFERS', 'INTERNET_TYPE', 'AVERAGE_MONTHLY_GB_DOWNLOAD', 'UNLIMITED_DATA', 'CONTRACT', 'PAYMENT_METHOD', 'MONTHLY_CHARGE', 'TOTAL_CHARGES','TOTAL_EXTRA_DATA_CHARGES']
        ...
        if output == 0:
            output = 'TELECOM CUSTOMER WILL CHURN'
        else:
            output = 'TELECOM CUSTOMER WILL CONTINUE ON THIS NETWORK'
